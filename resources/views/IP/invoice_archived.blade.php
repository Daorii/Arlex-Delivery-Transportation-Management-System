...
<head>
    ...
    <title>ADTMS - Archived Invoices</title>
...
<body class="bg-gray-50">
    <div class="flex h-screen overflow-hidden" x-data="archivedInvoicesData()">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <header class="bg-white border-b border-gray-200 px-8 py-3 flex items-center justify-between">
                <div class="flex items-center space-x-4 h-[55px] w-auto">
                    ...
                    <h1 class="text-2xl font-semibold text-gray-800">Archived Invoices</h1>
                </div>
            </header>

            <!-- Archived Invoices Content -->
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Net Total</th>
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="px-2 text-xs font-semibold 
                                                {{ $invoice['invoice_status'] === 'Draft' ? 'text-gray-600' : '' }}
                                                {{ $invoice['invoice_status'] === 'Sent' ? 'text-blue-600' : '' }}
                                                {{ $invoice['invoice_status'] === 'Partially Paid' ? 'text-yellow-600' : '' }}
                                                {{ $invoice['invoice_status'] === 'Fully Paid' ? 'text-green-600' : '' }}
                                                {{ $invoice['invoice_status'] === 'Overdue' ? 'text-red-600' : '' }}
                                                {{ $invoice['invoice_status'] === 'Cancelled' ? 'text-red-800' : '' }}">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <div class="flex space-x-2">
                                                <!-- Restore Button -->
                                                <button @click="selectedInvoiceId = {{ $invoice['invoice_id'] }}; selectedInvoiceNo = '{{ $invoice['invoice_no'] }}'; showRestoreModal = true"
                                                ...
                                                <!-- Delete Button -->
                                                <button @click="selectedInvoiceId = {{ $invoice['invoice_id'] }}; selectedInvoiceNo = '{{ $invoice['invoice_no'] }}'; showDeleteModal = true"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="px-6 py-12 text-center">
                                            ...
                                            <p class="mt-4 text-gray-500 text-sm">No archived invoices found.</p>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
                ...
                <div x-show="showRestoreModal" class="relative bg-white rounded-xl shadow-2xl max-w-lg w-full p-6">
                    <div class="flex items-start">
                        ...
                        <div class="ml-4 flex-1">
                            <h3 class="text-lg font-semibold text-gray-900 mb-2">Restore Invoice</h3>
                            <p class="text-sm text-gray-600">
                                Are you sure you want to restore invoice <span class="font-semibold" x-text="selectedInvoiceNo"></span>?
                            </p>
                        </div>
                    </div>
                    <div class="mt-6 flex justify-end space-x-3">
                        ...
                        <button @click="restoreInvoice()" class="px-4 py-2 rounded-lg bg-green-600 text-white hover:bg-green-700">Yes, Restore</button>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div x-show="showDeleteModal" class="relative bg-white rounded-xl shadow-2xl max-w-lg w-full p-6">
                    <div class="flex items-start">
                        ...
                        <div class="ml-4 flex-1">
                            ...
                            <p class="text-sm text-gray-600">
                                Are you sure you want to permanently delete invoice <span class="font-semibold" x-text="selectedInvoiceNo"></span>? <span class="text-red-600 font-semibold">This cannot be undone!</span>
                            </p>
                        </div>
                    </div>
                    <div class="mt-6 flex justify-end space-x-3">
                        <button @click="showDeleteModal = false" class="px-4 py-2 rounded-lg bg-gray-600 text-white hover:bg-gray-700">Cancel</button>
                        <button @click="deleteInvoice()" class="px-4 py-2 rounded-lg bg-red-600 text-white hover:bg-red-700">Yes, Delete</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        function archivedInvoicesData() {
            return {
                ...
                async restoreInvoice() {
                    try {
                        const response = await fetch(`/invoices/${this.selectedInvoiceId}/restore`, {
                            method: 'POST',
                            headers: {
                        ...
                        if (result.success) {
                            window.dispatchEvent(new CustomEvent('notify', { 
                                detail: { message: 'Invoice restored successfully!', type: 'success' } 
                            }));
                        ...
                        window.dispatchEvent(new CustomEvent('notify', { 
                            detail: { message: 'Error restoring invoice', type: 'error' } 
                        }));
                    }
                },
                ...
                async deleteInvoice() {
                    try {
                        const response = await fetch(`/invoices/${this.selectedInvoiceId}`, {
                            method: 'DELETE',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                        ...
                        if (result.success) {
                            window.dispatchEvent(new CustomEvent('notify', { 
                                detail: { message: 'Invoice permanently deleted!', type: 'success' } 
                            }));
                        ...
                        console.error('Error:', error);
                        window.dispatchEvent(new CustomEvent('notify', { 
                            detail: { message: 'Error deleting invoice', type: 'error' } 
                        }));
                    }
                }
            }
