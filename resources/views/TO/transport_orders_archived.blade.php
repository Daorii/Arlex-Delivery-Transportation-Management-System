...
<head>
    ...
    <title>ADTMS - Archived Transport Orders</title>
...
<body class="bg-gray-50">
    <div class="flex h-screen overflow-hidden" x-data="archivedTOData()">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <header class="bg-white border-b border-gray-200 px-8 py-3 flex items-center justify-between">
                <div class="flex items-center space-x-4 h-[55px] w-auto">
                    <a href="{{ route('transport-orders.index') }}" class="text-gray-500 hover:text-gray-700">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                        </svg>
                    </a>
                    <h1 class="text-2xl font-semibold text-gray-800">Archived Transport Orders</h1>
                </div>
            </header>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">TO Ref No</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">SIPA Ref No</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($archivedOrders as $order)
                                    <tr class="hover:bg-gray-50">
                                            ...
                                            <div class="flex flex-col space-y-1">
                                                @foreach($order['items'] as $item)
                                                    <div class="flex items-center space-x-2">
                                                        ...
                                                        <span class="px-2 py-1 rounded-full text-xs font-semibold {{ $item['type'] === 'Dry' ? 'bg-orange-100 text-orange-800' : 'bg-blue-100 text-blue-800' }}">
                                                            {{ $item['type'] }}
                                                        </span>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                            {{ $order['depot_from'] }} → {{ $order['depot_to'] }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="px-2 text-xs font-semibold {{ $order['verification_status'] === 'Approved' ? 'text-green-600' : ($order['verification_status'] === 'Declined' ? 'text-red-600' : 'text-yellow-600') }}">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <div class="flex space-x-2">
                                                <!-- Restore Button -->
                                                <button @click="selectedToRefNo = '{{ $order['to_ref_no'] }}'; showRestoreModal = true"
                                                ...
                                                <!-- Delete Button -->
                                                <button @click="selectedToRefNo = '{{ $order['to_ref_no'] }}'; showDeleteModal = true"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="px-6 py-12 text-center">
                                            ...
                                            <p class="mt-4 text-gray-500 text-sm">No archived transport orders found.</p>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
                ...
                <div x-show="showRestoreModal" class="relative bg-white rounded-xl shadow-2xl max-w-lg w-full p-6">
                    <div class="flex items-start">
                        ...
                        <div class="ml-4 flex-1">
                            <h3 class="text-lg font-semibold text-gray-900 mb-2">Restore Transport Order</h3>
                            <p class="text-sm text-gray-600">
                                Are you sure you want to restore <span class="font-semibold" x-text="selectedToRefNo"></span>?
                            </p>
                        </div>
                    </div>
                    <div class="mt-6 flex justify-end space-x-3">
                        ...
                        <button @click="restoreOrder()" class="px-4 py-2 rounded-lg bg-green-600 text-white hover:bg-green-700">Yes, Restore</button>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div x-show="showDeleteModal" class="relative bg-white rounded-xl shadow-2xl max-w-lg w-full p-6">
                    <div class="flex items-start">
                        ...
                        <div class="ml-4 flex-1">
                            ...
                            <p class="text-sm text-gray-600">
                                Are you sure you want to permanently delete <span class="font-semibold" x-text="selectedToRefNo"></span>? <span class="text-red-600 font-semibold">This cannot be undone!</span>
                            </p>
                        </div>
                    </div>
                    <div class="mt-6 flex justify-end space-x-3">
                        <button @click="showDeleteModal = false" class="px-4 py-2 rounded-lg bg-gray-600 text-white hover:bg-gray-700">Cancel</button>
                        <button @click="deleteOrder()" class="px-4 py-2 rounded-lg bg-red-600 text-white hover:bg-red-700">Yes, Delete</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        function archivedTOData() {
            return {
                ...
                async restoreOrder() {
                    try {
                        const response = await fetch(`/transport-orders/${this.selectedToRefNo}/restore`, {
                            method: 'POST',
                            headers: {
                        ...
                        if (result.success) {
                            this.showToastMessage('Transport Order restored successfully!', 'success');
                        ...
                        this.showToastMessage('Error restoring order', 'error');
                    }
                },

                async deleteOrder() {
                    try {
                        const response = await fetch(`/transport-orders/${this.selectedToRefNo}`, {
                            method: 'DELETE',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                        ...
                        if (result.success) {
                            this.showToastMessage('Transport Order permanently deleted!', 'success');
